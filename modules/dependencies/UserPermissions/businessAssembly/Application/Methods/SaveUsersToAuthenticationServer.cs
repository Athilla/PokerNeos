...
namespace Transversals.Business.UserPermissions.Application.Methods
{
    ...
    public class SaveUsersToAuthenticationServer : ISaveUsersToAuthenticationServer
    {
        ...
        /// <inheritdoc/>
        public async Task<UserPermissionsMethodResult> ExecuteAsync(UserAuthentication[] userInformationList)
        {
            if (!userInformationList.Any())
            {
                return new UserPermissionsMethodResult()
                {
                    Message = Resources.UserPermissions.SaveUsersToAuthenticationServerFailed,
                };
            }
